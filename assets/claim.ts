import { AccountInfo, Connection, Keypair, PublicKey, Transaction, TransactionInstruction, TransactionMessage, VersionedTransaction } from '@solana/web3.js';
...
async function claimReward(): Promise<void> {
    const claimer = Keypair.fromSecretKey(Uint8Array.from(privkey));

    const seed = decode(couponCode);
    const playerAddress = PublicKey.findProgramAddressSync([Buffer.from("L"), Buffer.from(seed)], programID);
    const playerBuffer = await connection.getAccountInfo(playerAddress[0]);
    const coupon = deserialize(LottoGameModel.getSchema(), LottoGameModel, playerBuffer!.data);

    const gameAddress = PublicKey.findProgramAddressSync(
        [Buffer.from("L"), Buffer.from(seed)],
        programID,
    );
