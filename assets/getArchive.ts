...
const draw_no = 3;
  ...
  const get_archive = async()=>{
    ...
    const wining_numbers_data = deserialize(WinningnumbersSchema,Winningnumbers,wining_numbers_acc!.data);

    const distribution_acc = await connection.getAccountInfo(distribution[0]);
    const distribution_data = deserialize(DistributionSchema,Distribution,distribution_acc!.data);

    const number_of_winners_acc = await connection.getAccountInfo(number_of_winners[0]);
    const number_of_winners_data = deserialize(NumberofwinnersSchema,Numberofwinners,number_of_winners_acc!.data);
    ...
    console.log("carry over to next week = "+carry_over_data.carryover +" lamports");
