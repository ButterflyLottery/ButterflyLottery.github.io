import {SignatureResult,SystemProgram,RpcResponseAndContext, Connection, Keypair, PublicKey,
  TransactionInstruction, TransactionMessage, VersionedTransaction,BlockheightBasedTransactionConfirmationStrategy 
} from "@solana/web3.js";
...
async function play(
  ...
  const account1_create_ix = SystemProgram.createAccount({
...
const account2_create_ix = SystemProgram.createAccount({
    fromPubkey: publicKey!,
...
if (term.draw_ticket_amount !== 0) {
    ...
    if ((playerRecordBuffer === null || playerRecordBuffer.lamports === null)) {
        const createPlayerRecord = SystemProgram.createAccountWithSeed({
        ...
        const message = new TransactionMessage({
          instructions: [account1_create_ix,account2_create_ix,createPlayerRecord,ix],
      ...
      const message = new TransactionMessage({
        instructions: [account1_create_ix,account2_create_ix,ix],
        payerKey: player.publicKey,
        recentBlockhash: (await connection.getLatestBlockhash()).blockhash
      }).compileToV0Message();
      tx = new VersionedTransaction(message);
...
const txConfirmation = await connection.confirmTransaction(confirmStrategy,'singleGossip');
