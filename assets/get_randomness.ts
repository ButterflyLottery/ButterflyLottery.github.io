import {
  ...
  const get_randomness_of_a_mainregister_and_a_subregister = async() => {
    ...
    //to derive final randomness in the time interval we make 
    //use the set that is not used
    // there are two sets of registers
    //no1 and no2
    console.log("Set of no"+rec_data.mcofweek+" is in use");

    const setofRegisters = 1;
    ...
    const mainregister_account = await connection.getAccountInfo(mainregister[0]);
    const mainregister_data = deserialize(MainRegisterSchema,MainRegister,mainregister_account!.data);
